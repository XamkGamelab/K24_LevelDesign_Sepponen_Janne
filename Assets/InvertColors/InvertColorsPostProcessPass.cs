...
[System.Serializable]
public class InvertColorsPostProcessPass : ScriptableRenderPass
{
    //NOTE: If changed to RTHandles, should the be initialized?
    ...
    public InvertColorsPostProcessPass()
    {
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        var materials = InvertColorsMaterialPointer.Instance;
        if (materials == null)
        {
        ...
        CommandBuffer cmd = CommandBufferPool.Get("Invert Colors Post Processing");
        cmd.Clear();
        ...
        //---Custom effect here---
        var customEffect = stack.GetComponent<InvertColorsComponent>();
        // Only process if the effect is active
        if (customEffect.IsActive())
        {
            var material = materials.InvertColorsMaterial;

            // P.s. optimize by caching the property ID somewhere else            
            material.SetColor(Shader.PropertyToID("MultiplyColor"), customEffect.MultiplyColor.value);
            material.SetFloat(Shader.PropertyToID("InvertColorsIntensity"), customEffect.InvertColorsIntensity.value);
